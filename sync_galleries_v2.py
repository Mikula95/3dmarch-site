...
for html_file, rel_folder in target_folders.items():
    ...
    # Find gallery images
    gallery_files = [f for f in all_files if 'hero' not in f.lower() and f.lower() != 'hero_mobile.jpg']
    ...
    if hero_img:
        ...
        # User requested to leave it empty if no hero is found
        content = hero_regex.sub(rf"\g<1>''\g<3>", content)

    # 2. Update gallery
    gallery_html = ""
    # Make sure we don't clear galleries on index/category pages inadvertently
    if html_file not in ['index.html', 'architecture.html', 'archviz.html', 'productvisualization.html', 'virtualrealms.html', 'droneservices.html']:
        ...
        # Replace the contents of <div class="gallery-dynamic">
